...
type TCRC2 = TransformCanvasRenderingContext2D;
...
export function toTransformedContext(
  ...
  (ctx as TCRC2).transformedPoint = function (x, y) {
    pt.x = x;
    pt.y = y;
  ...
  (ctx as TCRC2).clearCanvas = function () {
  ...
  (ctx as TCRC2).beginPan = function (e) {
  ...
  (ctx as TCRC2).endPan = function () {
    dragStart = undefined;
  };

  let zoom = 0;

  (ctx as TCRC2).zoom = function (
    amount,
    zoomFactor = 1.1,
    center?: { x: number; y: number }
  ) {
